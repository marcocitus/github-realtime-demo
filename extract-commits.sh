...
if [ "$num_processed" -eq "$num_processed" ] && [ "$num_processed" -gt 0 ] ; then
  ...
  ispeed=$(psql -tAX -c "SELECT CASE WHEN count(*) > 1 THEN pg_size_pretty((60*sum(events)/(extract(epoch from max(extract_time)-min(extract_time))))::bigint*(SELECT sum(size)/sum(events) FROM (SELECT * FROM ingest ORDER BY ingest_time DESC LIMIT 10) a)::bigint) ELSE '17 GB' END FROM (SELECT * FROM extract WHERE extract_time >= now() - interval '20 minutes' ORDER BY extract_time DESC LIMIT 10) extracts;")
