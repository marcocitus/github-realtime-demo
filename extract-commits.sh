...
if [ "$num_processed" -lt 1 ] ; then
  ...
  ispeed=$(psql -tAX -c "SELECT pg_size_pretty((60*sum(events)/(extract(epoch from max(extract_time)-min(extract_time))))::bigint*(SELECT sum(size)/sum(events) FROM (SELECT * FROM ingest ORDER BY ingest_time DESC LIMIT 10) a)::bigint) FROM (SELECT * FROM extract WHERE extract_time >= now() - interval '20 minutes' ORDER BY extract_time DESC LIMIT 10) extracts")
